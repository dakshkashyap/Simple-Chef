...
const RecipeCard = ({ image, title, description, rating, link }) => {
  const renderStars = (rating) => {
    ...
    for (let i = 0; i < 5; i++) {
      stars.push(
        <span key={i} className={`recipe-card__star ${i < rating ? 'recipe-card__star--filled' : ''}`}>
          &#9733;
        </span>
      );
    }
    ...
    <div className="recipe-card">
      <IconContext.Provider value={{ size: "2em" }}>
        <div className="recipe-card__icons">
            <span className="recipe-card__icon recipe-card__icon--like"><FcLike /></span>
        </div>
      ...
      <div className="recipe-card__content">
        ...
        <a href={link} className="recipe-card__cta">View Details</a>
      </div>
    </div>
  );
};
